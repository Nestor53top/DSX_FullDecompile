...
public class TrayStateChanger : IEnableLogger
{
	...
	public unsafe void RemoveDeadEntries(List<string> executablesInPackage, string rootAppDirectory, string currentAppVersion)
	{
		...
		try
		{
			array = (byte[])Registry.GetValue("HKEY_CURRENT_USER\\Software\\Classes\\Local Settings\\Software\\Microsoft\\Windows\\CurrentVersion\\TrayNotify", "IconStreams", (object)new byte[1]);
		}
		catch (Exception ex)
		{
		...
		try
		{
			...
			Registry.SetValue("HKEY_CURRENT_USER\\Software\\Classes\\Local Settings\\Software\\Microsoft\\Windows\\CurrentVersion\\TrayNotify", "IconStreams", (object)array3);
		}
		catch (Exception ex2)
		{
