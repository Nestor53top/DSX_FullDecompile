...
internal class ZlibBaseStream : Stream
{
	...
	private void finish()
	{
		...
		if (_streamMode == StreamMode.Writer)
		{
			...
			if (_flavor == ZlibStreamFlavor.GZIP)
			{
				...
				_stream.Write(DataConverter.LittleEndian.GetBytes(crc32Result), 0, 4);
				int value = (int)(crc.TotalBytesRead & 0xFFFFFFFFu);
				_stream.Write(DataConverter.LittleEndian.GetBytes(value), 0, 4);
			}
		}
		else
		{
			...
			int @int = DataConverter.LittleEndian.GetInt32(array, 0);
			int crc32Result2 = crc.Crc32Result;
			int int2 = DataConverter.LittleEndian.GetInt32(array, 4);
			int num4 = (int)(_z.TotalBytesOut & 0xFFFFFFFFu);
			if (crc32Result2 != @int)
			{
				throw new ZlibException($"Bad CRC32 in GZIP stream. (actual({crc32Result2:X8})!=expected({@int:X8}))");
			}
			if (num4 != int2)
			{
				throw new ZlibException($"Bad size in GZIP stream. (actual({num4})!=expected({int2}))");
			}
		}
	}
	...
	private int _ReadAndValidateGzipHeader()
	{
		...
		case 10:
		{
			...
			_GzipMtime = epoch.AddSeconds(@int);
			num += num2;
			if ((array[3] & 4) == 4)
			{
