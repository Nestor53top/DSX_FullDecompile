...
public class OptionSet : KeyedCollection<string, Option>
{
	...
	private sealed class ActionOption : Option
	{
		...
		public ActionOption(string prototype, string description, int count, Action<OptionValueCollection> action)
			: this(prototype, description, count, action, hidden: false)
		{
		}
	...
	private class ArgumentEnumerator : IEnumerable<string>, IEnumerable
	{
		...
		public IEnumerator<string> GetEnumerator()
		{
			do
			{
				IEnumerator<string> enumerator = sources[sources.Count - 1];
	...
	{
		return Add(prototype, description, action, hidden: false);
	}
	...
	public List<string> Parse(IEnumerable<string> arguments)
	{
		...
		ArgumentEnumerator argumentEnumerator = new ArgumentEnumerator(arguments);
		foreach (string item in argumentEnumerator)
		{
			...
			{
				Unprocessed(list, def, optionContext, item);
			}
			else if (!AddSource(argumentEnumerator, item) && !Parse(item, optionContext))
			{
				Unprocessed(list, def, optionContext, item);
			}
		}
	...
	protected virtual bool Parse(string argument, OptionContext c)
	{
		...
		if (ParseBundledValue(flag, string.Concat(new string[1] { name + sep + value }), c))
		{
			return true;
		}
		return false;
	...
	public void WriteOptionDescriptions(TextWriter o)
	{
		using (IEnumerator<Option> enumerator = GetEnumerator())
		{
			while (enumerator.MoveNext())
			{
				...
				{
					WriteDescription(o, current.Description, "", 80, 80);
				}
				else if (WriteOptionPrototype(o, current, ref written))
				{
					if (written < 29)
					{
					...
					WriteDescription(o, current.Description, new string(' ', 31), 51, 49);
				}
			}
		}
		foreach (ArgumentSource source in sources)
		{
			...
			if (names != null && names.Length != 0)
			{
				...
				WriteDescription(o, source.Description, new string(' ', 31), 51, 49);
			}
		}
	}
