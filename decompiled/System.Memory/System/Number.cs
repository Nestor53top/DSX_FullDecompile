...
internal static class Number
{
	...
	public static void RoundNumber(ref System.NumberBuffer number, int pos)
	{
	...
	internal static bool NumberBufferToDouble(ref System.NumberBuffer number, out double value)
	{
	...
	public unsafe static bool NumberBufferToDecimal(ref System.NumberBuffer number, ref decimal value)
	{
		System.MutableDecimal source = default(System.MutableDecimal);
		...
		else
		{
			...
			{
				System.DecimalDecCalc.DecMul10(ref source);
				if (*ptr != 0)
				{
					System.DecimalDecCalc.DecAddInt32(ref source, (uint)(*(ptr++) - 48));
				}
				num--;
			}
			if (*(ptr++) >= 53)
			{
				...
				if (flag)
				{
					System.DecimalDecCalc.DecAddInt32(ref source, 1u);
		...
		value = Unsafe.As<System.MutableDecimal, decimal>(ref source);
		return true;
	}

	public static void DecimalToNumber(decimal value, ref System.NumberBuffer number)
	{
		ref System.MutableDecimal reference = ref Unsafe.As<decimal, System.MutableDecimal>(ref value);
		Span<byte> digits = number.Digits;
		number.IsNegative = reference.IsNegative;
		int num = 29;
		while ((reference.Mid != 0) | (reference.High != 0))
		{
			uint num2 = System.DecimalDecCalc.DecDivMod1E9(ref reference);
			for (int i = 0; i < 9; i++)
			{
	...
	private unsafe static double NumberToDouble(ref System.NumberBuffer number)
	{
