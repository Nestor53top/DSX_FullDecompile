...
public static class MemoryExtensions
{
	...
	public static bool SequenceEqual<T>(this Span<T> span, ReadOnlySpan<T> other) where T : IEquatable<T>
	{
		...
		if (default(T) != null && IsTypeComparableAsBytes<T>(out var size))
		{
			...
			{
				return System.SpanHelpers.SequenceEqual(ref Unsafe.As<T, byte>(ref MemoryMarshal.GetReference(span)), ref Unsafe.As<T, byte>(ref MemoryMarshal.GetReference(other)), (System.NUInt)length * size);
			}
			return false;
		}
		if (length == other.Length)
	...
	public static bool StartsWith<T>(this Span<T> span, ReadOnlySpan<T> value) where T : IEquatable<T>
	{
		...
		if (default(T) != null && IsTypeComparableAsBytes<T>(out var size))
		{
			...
			{
				return System.SpanHelpers.SequenceEqual(ref Unsafe.As<T, byte>(ref MemoryMarshal.GetReference(span)), ref Unsafe.As<T, byte>(ref MemoryMarshal.GetReference(value)), (System.NUInt)length * size);
			}
			return false;
		}
		if (length <= span.Length)
	...
	public static bool EndsWith<T>(this Span<T> span, ReadOnlySpan<T> value) where T : IEquatable<T>
	{
		...
		if (default(T) != null && IsTypeComparableAsBytes<T>(out var size))
		{
			if (length2 <= length)
			{
				return System.SpanHelpers.SequenceEqual(ref Unsafe.As<T, byte>(ref Unsafe.Add(ref MemoryMarshal.GetReference(span), length - length2)), ref Unsafe.As<T, byte>(ref MemoryMarshal.GetReference(value)), (System.NUInt)length2 * size);
			}
			return false;
		}
		if (length2 <= length)
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private static bool IsTypeComparableAsBytes<T>(out System.NUInt size)
	{
	...
	public static ReadOnlySpan<char> AsSpan(this string text)
	{
		...
		return new ReadOnlySpan<char>(Unsafe.As<System.Pinnable<char>>(text), StringAdjustment, text.Length);
	}
	...
	public static ReadOnlySpan<char> AsSpan(this string text, int start)
	{
		...
		return new ReadOnlySpan<char>(Unsafe.As<System.Pinnable<char>>(text), StringAdjustment + start * 2, text.Length - start);
	}
	...
	public static ReadOnlySpan<char> AsSpan(this string text, int start, int length)
	{
		...
		return new ReadOnlySpan<char>(Unsafe.As<System.Pinnable<char>>(text), StringAdjustment + start * 2, length);
	}
	...
	private unsafe static IntPtr MeasureStringAdjustment()
	{
		...
		{
			return Unsafe.ByteOffset(ref Unsafe.As<System.Pinnable<char>>(text).Data, ref Unsafe.AsRef<char>(source));
		}
	}
}
