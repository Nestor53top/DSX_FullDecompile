...
public static class Utf8Formatter
{
	...
	{
		TimeSpan offset = System.Buffers.Text.Utf8Constants.s_nullUtcOffset;
	...
	public static bool TryFormat(DateTime value, Span<byte> destination, out int bytesWritten, StandardFormat format = default(StandardFormat))
	{
		return System.Buffers.Text.FormattingHelpers.GetSymbolOrDefault(in format, 'G') switch
		{
			...
			'O' => TryFormatDateTimeO(value, System.Buffers.Text.Utf8Constants.s_nullUtcOffset, destination, out bytesWritten), 
			'G' => TryFormatDateTimeG(value, System.Buffers.Text.Utf8Constants.s_nullUtcOffset, destination, out bytesWritten), 
			_ => System.ThrowHelper.TryFormatThrowFormatException(out bytesWritten), 
		};
	}
	...
	{
		int num = 19;
		if (offset != System.Buffers.Text.Utf8Constants.s_nullUtcOffset)
		{
			num += 7;
		}
	...
	public static bool TryFormat(decimal value, Span<byte> destination, out int bytesWritten, StandardFormat format = default(StandardFormat))
	{
		...
		case 'g':
		{
			...
			System.NumberBuffer number3 = default(System.NumberBuffer);
		...
		case 'f':
		{
			System.NumberBuffer number2 = default(System.NumberBuffer);
		...
		case 'e':
		{
			System.NumberBuffer number = default(System.NumberBuffer);
	...
	private static bool TryFormatDecimalE(ref System.NumberBuffer number, Span<byte> destination, out int bytesWritten, byte precision, byte exponentSymbol)
	{
	...
	private static bool TryFormatDecimalF(ref System.NumberBuffer number, Span<byte> destination, out int bytesWritten, byte precision)
	{
		int scale = number.Scale;
		ReadOnlySpan<byte> readOnlySpan = number.Digits;
	...
	private static bool TryFormatDecimalG(ref System.NumberBuffer number, Span<byte> destination, out int bytesWritten)
	{
		int scale = number.Scale;
		ReadOnlySpan<byte> readOnlySpan = number.Digits;
