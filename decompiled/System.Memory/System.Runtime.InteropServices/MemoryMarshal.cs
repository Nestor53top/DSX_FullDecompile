...
public static class MemoryMarshal
{
	...
	public static Span<byte> AsBytes<T>(Span<T> span) where T : struct
	{
		...
		return new Span<byte>(Unsafe.As<System.Pinnable<byte>>(span.Pinnable), span.ByteOffset, length);
	}
	...
	public static ReadOnlySpan<byte> AsBytes<T>(ReadOnlySpan<T> span) where T : struct
	{
		...
		int length = checked(span.Length * Unsafe.SizeOf<T>());
		return new ReadOnlySpan<byte>(Unsafe.As<System.Pinnable<byte>>(span.Pinnable), span.ByteOffset, length);
	}
	...
	public static Span<TTo> Cast<TFrom, TTo>(Span<TFrom> span) where TFrom : struct where TTo : struct
	{
		...
		checked
		{
			...
			return new Span<TTo>(Unsafe.As<System.Pinnable<TTo>>(span.Pinnable), span.ByteOffset, length);
		}
	}
	...
	public static ReadOnlySpan<TTo> Cast<TFrom, TTo>(ReadOnlySpan<TFrom> span) where TFrom : struct where TTo : struct
	{
		...
		{
			int length = (int)unchecked(checked(unchecked((long)span.Length) * unchecked((long)Unsafe.SizeOf<TFrom>())) / Unsafe.SizeOf<TTo>());
			return new ReadOnlySpan<TTo>(Unsafe.As<System.Pinnable<TTo>>(span.Pinnable), span.ByteOffset, length);
		}
	}
}
