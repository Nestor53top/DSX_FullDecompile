...
internal sealed class LinuxSerialStream : SerialStream
{
	...
	public unsafe override int Read(byte[] buffer, int offset, int count)
	{
		...
		fixed (byte* ptr = buffer)
		{
			...
			do
			{
				...
				num = NativeMethods.retry(() => NativeMethods.poll(ref fd, (IntPtr)1, GetTimeout(startTime, readTimeout)));
				if (num < 0)
				{
	...
	public unsafe override void Write(byte[] buffer, int offset, int count)
	{
		...
		fixed (byte* ptr = buffer)
		{
			...
			while (num < count)
			{
				...
				int num2 = NativeMethods.retry(() => NativeMethods.poll(ref fd, (IntPtr)1, GetTimeout(startTime, writeTimeout)));
				if (num2 < 0)
				{
	...
	{
		uint c_iflag = _newSettings.c_iflag;
		c_iflag = 0u;
		_newSettings.c_iflag = c_iflag;
		uint c_cflag = _newSettings.c_cflag;
		c_cflag &= 0xFFFFFFBFu;
		c_cflag &= 0xFFFFFFCFu;
		c_cflag &= 0xFFFFFEFFu;
		c_cflag |= 0x30;
		...
		c_cflag |= 0x800;
		c_cflag &= 0x7FFFFFFF;
		_newSettings.c_cflag = c_cflag;
		uint c_oflag = _newSettings.c_oflag;
		c_oflag &= 0xFFFFFFFEu;
		_newSettings.c_oflag = c_oflag;
	...
	private void UpdateSettings()
	{
		lock (_lock)
		{
			...
			if (handle >= 0 && _settingsChanged)
			{
				...
				int num = NativeMethods.retry(() => NativeMethods.cfsetspeed(ref _newSettings, (uint)Math.Max(1, baudRate)));
				if (num < 0)
				{
				...
				uint c_cflag = _newSettings.c_cflag;
				c_cflag &= 0xFFFFFFCFu;
				c_cflag = ((dataBits != 7) ? (c_cflag | 0x30) : (c_cflag | 0x20));
				c_cflag &= 0xFFFFFCFFu;
				switch (parity)
				{
				...
				num = NativeMethods.retry(() => NativeMethods.tcflush(handle, 0));
				if (num < 0)
				{
