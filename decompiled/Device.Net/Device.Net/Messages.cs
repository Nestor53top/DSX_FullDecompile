...
public static class Messages
{
	...
	public static string SuccessMessageWriteAndReadCalled => "Successfully called WriteAndReadAsync";

	public static string WarningMessageOpeningInReadonlyMode(string deviceId)
	{
		return "Opening device " + deviceId + " in read only mode.";
	}

	public static string GetErrorMessageCantConnect(string deviceId)
	{
		return "Could not connect to device with Device Id " + deviceId + ". Check that the package manifest has been configured to allow this device.";
	}
	...
	{
		return string.Format("No bulk {0} pipes found. Interrupt pipes to be used instead. Interface Number {1}", isRead ? "read" : "write", interfaceNumber);
	}
}
